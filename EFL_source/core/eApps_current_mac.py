...
class DeepMLPNextPos(nn.Module):
    def __init__(self, seq_len):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(d_h,  d_h), nn.GELU(),
            nn.Linear(d_h,  d_h), nn.GELU(),
            nn.Linear(d_h,  2)   # (dx, dy)
        )

    def forward(self, x):              # x: (B, T, 2)
        z = x.reshape(x.size(0), -1)   # (B, 2*T)
...
need_train = False
# Reuse the deep MLP trainer you already added
if need_train:
...
def mac_scheduling(connections, channel_gains, channel_gains_real, total_p_val, t, T_max , n_bits, coef, importance_weight, orus_by_idx):
    energy_J = 0
    counter = 0
    ...
    for b in orus_by_idx:
        ...
        if num_users_per_oru > 0:
            ...
            # viol_penalty = cp.sum(cp.multiply(s,importance_weight_oru))
            # eps = 1e-6  # choose your tolerance in NAT/s
            # violation_proxy = cp.pos(s )   # elementwise
            # viol_cost = cp.sum(cp.multiply(importance_weight_oru, violation_proxy))
            #

            # Exact Mixed Integer
            # z = cp.Variable(num_users_per_oru, boolean=True)
            # cons += [Rnat >= cp.multiply(required_rate_nat, z)]
            # objective = cp.Maximize(importance_weight_oru @ z)

            #Approximation
            lambda_R = 0.0
            total_energy = cp.sum(P)
            # cons += [Rnat >= cp.multiply(required_rate_nat,s)]
            objective = cp.Maximize(cp.sum(Rnat)-total_energy*lambda_R)
...
def mobility_handler(channel_gains, num_orus, num_users, users_per_odu, orus_per_odu):
    ...
    objective = cp.Maximize(cp.sum(Rnat)-lambda_energy*total_energy)
...
for gr in range(num_global_rounds):
        ...
        for d in range(num_odus):
            ...
            if num_user_per_odu[gr, gt][d]>0 and num_oru_per_odu[gr, gt][d]>0:
                ...
                status, B_val, P_val, Rbps_val = mobility_handler(channel_gains = channel_gains[gr, gt * near_rt_time_span], num_orus = num_oru_per_odu[gr, gt][d],
                                                                  ...
                                                                  orus_per_odu = idx_oru_by_group[gr,gt][d])
                ...
                if status not in ("optimal", "optimal_inaccurate"):
                    ...
                    for t in range(near_rt_time_span):
                        ...
                        e, counter, computation_time = mac_scheduling(connections, channel_gains = channel_gains[gr, gt * near_rt_time_span + t], channel_gains_real= channel_gains_real[gr, gt * near_rt_time_span + t], total_p_val= P_val_total,
                                                                      ...
                                                                      orus_by_idx = idx_oru_by_group[gr, gt][d])
                    ...
                    #     for _ in range(2):
                    #         e, counter, computation_time = mac_scheduling(connections, channel_gains = channel_gains[gr, gt * near_rt_time_span + t], channel_gains_real= channel_gains_real[gr, gt * near_rt_time_span + t], total_p_val = P_val_total,
                    #                                                       t=gt * near_rt_time_span + t - 1, T_max=T_max + time_frame, n_bits = n_bits, coef= coef, importance_weight = importance_weight,
                    #                                                       orus_by_idx = idx_oru_by_group[gr, gt][d])
                    #         energy_J += e
                    #         print(f"total successful users: {counter}")
        # T_max = T_max - near_rt_time_span * time_frame
    ...
    save_pickle({"success_status": success_status}, "results/EVFL/success_status_cur_mac.pkl")
    save_pickle({"energy_statistics": energy_statistics}, "results/EVFL/energy_statistics_cur_mac.pkl")
    save_pickle({"mac_latency_statistics": mac_latency_statistics},"results/EVFL/mac_latency_statistics_cur_mac.pkl")
    save_pickle({"near_rt_latency_statistics": near_rt_latency_statistics},"results/EVFL/near_rt_latency_statistics_cur_mac.pkl")
